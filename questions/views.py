...
def take_quiz(request, quiz_id):
    ...
    previous_attempt = Result.objects.filter(student=request.user, quiz=quiz).first()

    if previous_attempt:
        if previous_attempt.score >= 8:
            return render(
                request,
                "questions/already_attempted.html",
                {"quiz": quiz, "result": previous_attempt, "can_retake": False},
            )
        else:
            return render(
                request,
                "questions/already_attempted.html",
                {"quiz": quiz, "result": previous_attempt, "can_retake": True},
            )

    return render(
        request,
        "questions/take_quiz.html",
        {"quiz": quiz, "questions": questions, "time_limit": quiz.time_limit * 60},
    )
...
def submit_quiz(request, quiz_id):
    ...
    has_descriptive = questions.filter(question_type="descriptive").exists()
    ...
    for question in questions:
        if question.question_type == "mcq":
            ...
            descriptive_answer = request.POST.get(f"question_{question.id}", "")
            StudentAnswer.objects.create(
                result=result,
                question=question,
