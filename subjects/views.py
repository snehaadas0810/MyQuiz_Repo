...
def subject_detail(request, subject_id):
    ...
    for quiz in quizzes:
        result = (
            Result.objects.filter(student=request.user, quiz=quiz)
            .order_by("-date_taken")
            .first()
        )
    ...
    return render(
        ...
        {"subject": subject, "quiz_data": quiz_data},
    )
    ...
    if request.method == "POST":
        ...
        if not title:
            ...
            quiz = Quiz.objects.create(
                title=title, subject=subject, time_limit=time_limit
            )

            messages.success(request, "Quiz created successfully ✅")
            return redirect("add_question", quiz_id=quiz.id)

    return render(request, "subjects/create_quiz.html", {"subject": subject})
...
def add_question(request, quiz_id):
    ...
    return render(
        request, "subjects/add_question.html", {"quiz": quiz, "questions": questions}
    )
